  ...
  useEffect(() => {
    ...
    hands.onResults((results) => {
      ...
      if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        const indexTip = lms[8]; // Index finger tip
        const thumbTip = lms[4]; // Thumb tip

        const pinchDistance = distance(indexTip, thumbTip);

        // Use indexTip.x and indexTip.y for pencil position
        ...
        if (pinchDistance < PINCH_THRESHOLD) {
          if (!drawing) {
            ...
            lastX = smoothX; // Initialize lastX and lastY to the current smoothed position
            lastY = smoothY;
          }
          drawing = true;
          setGesture("Drawing Mode");
          if (pencil) {
            pencil.style.left = `${smoothX - 10}px`; // Adjust as needed
            pencil.style.top = `${smoothY - 10}px`; // Adjust as needed
          ...
          lastX = null; // Reset lastX and lastY when not drawing
          lastY = null;
        }
        ...
        // If no hand is detected, hide the pencil
        if (pencil) {
          pencil.style.display = "none";
        }
        setGesture("-");
        ...
        <div className="flex-1 flex justify-center items-center h-full px-6">
          <div className="relative w-full h-[80%]  bg-mainColor rounded-3xl">
            ...
            <p
              ...
              src={pencilImage}
              alt="Pencil"
              className="absolute  z-10 size-10 bg-amber-400"
            />
          </div>
        </div>
